...
import { CreateManifestButton } from '@/components/shared/create-manifest-button';
import { 
  Table, 
  TableBody, 
  TableCell, 
...
export const metadata = {
  title: 'Manifest Pengiriman - Architax PBB',
};

// Renders a high-fidelity semantic status badge for manifest status
function renderStatusBadge(status: ManifestStatus) {
  let bg = '';
  let dotColor = '';
  ...
  const isPulse = status === 'WAITING_SIGNATURE' || status === 'DRAFT';
...
export default async function ManifestListPage({ searchParams }: PageProps) {
  ...
  const result = await ManifestService.findFiltered({
    page: currentPage,
    limit,
  });
  ...
  const isShipper = session.user.role === 'STAF_PENGIRIM';
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 font-display tracking-tight">Manifest Pengiriman</h1>
          <p className="text-xs text-slate-500 mt-1 font-medium">
            Kelola manifest pengiriman berkas bundle ke kantor pusat
          </p>
        </div>
        {isShipper && <CreateManifestButton />}
      </div>
        ...
        <Table>
          <TableHeader className="bg-slate-50/50 border-b border-slate-100">
            <TableRow className="border-b border-slate-100 hover:bg-slate-50/50">
              <TableHead className="font-bold text-slate-500 text-[10px] uppercase tracking-wider w-[180px] py-3">No. Manifest</TableHead>
              <TableHead className="font-bold text-slate-500 text-[10px] uppercase tracking-wider py-3">Jumlah Bundle</TableHead>
              <TableHead className="font-bold text-slate-500 text-[10px] uppercase tracking-wider w-[160px] py-3">Status Manifest</TableHead>
              ...
              <TableHead className="font-bold text-slate-500 text-[10px] uppercase tracking-wider text-right w-[150px] py-3">Kanban Board</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {items.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="h-32 text-center text-slate-400 font-bold text-sm">
                  Belum ada manifest dibuat. Klik tombol di kanan atas untuk membuat manifest baru.
                </TableCell>
              </TableRow>
            ) : (
              items.map((item) => (
                <TableRow key={item.id} className="border-b border-slate-100 hover:bg-slate-50/30 transition-colors duration-150">
                  <TableCell className="font-bold text-slate-900 font-display">{item.manifestNumber}</TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    <Link
                      href={`/manifest/shipping?id=${item.id}`}
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t border-slate-100 bg-white">
            <p className="text-xs text-slate-500 font-bold select-none">
              Menampilkan {items.length} dari {total} manifest
            </p>
            <div className="flex items-center gap-3">
              <Link
                href={currentPage <= 1 ? '#' : `/manifest?page=${currentPage - 1}`}
              ...
              <Link
                href={currentPage >= totalPages ? '#' : `/manifest?page=${currentPage + 1}`}
                className={`border border-slate-200 hover:bg-slate-50 hover:text-slate-900 rounded-xl h-9 px-4 font-bold text-xs flex items-center gap-1.5 text-slate-700 transition-all duration-150 active:scale-95 cursor-pointer shadow-sm ${
